"""LightGBM with DART boosting (drops trees during training) for ensemble diversity."""
import sys, time
import numpy as np
import pandas as pd
import polars as pl
...
# DART: random tree dropouts during training - different model class
params = {
...
for fold, (tr_idx, va_idx) in enumerate(skf.split(X, strat_key)):
    ...
    m = lgb.train(params, dtr, num_boost_round=1500)  # DART has no early stopping
