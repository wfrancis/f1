"""v18: combine v15 add_orig + fastf1 + TE features. The "everything" model."""
import sys
import time
import numpy as np
import pandas as pd
...
for fold, (tr, va) in enumerate(skf.split(train_pd, strat_key)):
    fold_ids[va] = fold
...
te_ff = {name: np.zeros(len(ff_pd)) for name, _ in te_keys_list}
...
for name, keys in te_keys_list:
    ...
    agg['te'] = (agg['sum'] + SMOOTH * global_mean) / (agg['count'] + SMOOTH)
    te_test[name] = test_pd[keys].merge(agg[keys + ['te']], on=keys, how='left')['te'].fillna(global_mean).values
    te_orig[name] = orig_pd[keys].merge(agg[keys + ['te']], on=keys, how='left')['te'].fillna(global_mean).values
    te_ff[name] = ff_pd[keys].merge(agg[keys + ['te']], on=keys, how='left')['te'].fillna(global_mean).values

for name in te_oof:
    train_pd[name] = te_oof[name]
    test_pd[name] = te_test[name]
    orig_pd[name] = te_orig[name]
    ff_pd[name] = te_ff[name]
...
for c in cat_cols:
    cats = pd.concat([X[c].astype(str), X_orig[c].astype(str), X_test[c].astype(str), X_ff[c].astype(str)], ignore_index=True).astype(str).unique()
...
for fold, (tr_idx, va_idx) in enumerate(skf.split(X, strat_key)):
    ...
    X_tr = pd.concat([X.iloc[tr_idx], X_orig, X_ff], ignore_index=True)
    y_tr = np.concatenate([y[tr_idx], y_orig, y_ff])
