"""v19: CatBoost WITH Driver feature included (per top-54 user advice).
CatBoost's target-counter encoding might use Driver productively where LGBM overfits."""
import sys, time
import numpy as np
import pandas as pd
import polars as pl
...
OUT_NAME = f"v19_cat_driver_s{SEED}"
t0 = time.time()
...
print(f"features: {len(features)}, cat_idx: {cat_idx} (Driver INCLUDED)", flush=True)
...
for fold, (tr_idx, va_idx) in enumerate(skf.split(X, strat_key)):
    ...
    print(f"    fold {fold+1} AUC: {auc:.6f} (best_iter {model.get_best_iteration()})", flush=True)
