"""Small CatBoost on (comp + orig) — fast config to actually finish."""
import sys
import time
import numpy as np
import pandas as pd
...
OUT_NAME = f"v15_cat_small_s{SEED}"
t0 = time.time()
...
for fold, (tr_idx, va_idx) in enumerate(skf.split(X, strat_key)):
    ...
    # SMALL: depth 5, 800 iter, lr 0.10
    model = CatBoostClassifier(
        iterations=800, learning_rate=0.10, depth=5,
