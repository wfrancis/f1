"""
v15_te: combine v15 (add orig as training rows) with v13 (target encoding features).
Both ideas independently gave OOF lift; combination might compound.
"""
import sys
import time
import numpy as np
...
# === OOF Target Encoding (computed on train only, applied to train via OOF, test via overall) ===
print(f"computing target encoding ({time.time()-t0:.0f}s)", flush=True)
te_keys_list = [
    ('TE_lsc', ['Race', 'Year', 'LapNumber', 'Stint', 'Compound']),
    ('TE_lc', ['Race', 'Year', 'LapNumber', 'Compound']),
    ('TE_l', ['Race', 'Year', 'LapNumber']),
    ('TE_st', ['Race', 'Year', 'Stint', 'TyreLife']),
    ('TE_sc', ['Race', 'Year', 'Stint', 'Compound']),
]
SMOOTH = 10
global_mean = y.mean()

# OOF strat key for TE
strat_key = (train_pd["PitNextLap"].astype(int).astype(str) + "_" + train_pd["Year"].astype(str)).values
N_SPLITS = 5
skf = StratifiedKFold(n_splits=N_SPLITS, shuffle=True, random_state=SEED)

te_oof = {name: np.zeros(len(train_pd)) for name, _ in te_keys_list}
te_test = {name: np.zeros(len(test_pd)) for name, _ in te_keys_list}
te_orig = {name: np.zeros(len(orig_pd)) for name, _ in te_keys_list}

# For OOF on train: compute TE on tr fold, apply to val fold
fold_ids = np.zeros(len(train_pd), dtype=int)
for fold, (tr_idx, va_idx) in enumerate(skf.split(train_pd, strat_key)):
    fold_ids[va_idx] = fold

for fold in range(N_SPLITS):
    tr_idx = fold_ids != fold
    va_idx = fold_ids == fold
    fold_train = train_pd[tr_idx]
    for name, keys in te_keys_list:
        agg = fold_train.groupby(keys)['PitNextLap'].agg(['sum', 'count']).reset_index()
        agg['te'] = (agg['sum'] + SMOOTH * global_mean) / (agg['count'] + SMOOTH)
        # apply to val rows
        val = train_pd.iloc[va_idx][keys].merge(agg[keys + ['te']], on=keys, how='left')
        te_oof[name][va_idx] = val['te'].fillna(global_mean).values

# For test and orig: use full train TE
for name, keys in te_keys_list:
    agg = train_pd.groupby(keys)['PitNextLap'].agg(['sum', 'count']).reset_index()
    agg['te'] = (agg['sum'] + SMOOTH * global_mean) / (agg['count'] + SMOOTH)
    test_v = test_pd[keys].merge(agg[keys + ['te']], on=keys, how='left')
    te_test[name] = test_v['te'].fillna(global_mean).values
    orig_v = orig_pd[keys].merge(agg[keys + ['te']], on=keys, how='left')
    te_orig[name] = orig_v['te'].fillna(global_mean).values

for name in te_oof:
    train_pd[name] = te_oof[name]
    test_pd[name] = te_test[name]
    orig_pd[name] = te_orig[name]

num_cols = base_num_cols + list(te_oof.keys())
features = cat_cols + num_cols
print(f"features: {len(features)} (with {len(te_keys_list)} TE)", flush=True)
...
params = {
    ...
    "lambda_l2": 1.0, "verbose": -1, "n_jobs": 2, "seed": SEED,
}
