"""
v16: train on (comp + orig + fastf1) combined.
Adds 9,782 fastf1 rows from 11 missing sessions to the training pool.
"""
import sys
import time
import numpy as np
...
print(f"train: {train.shape}, orig: {orig.shape}, fastf1: {fastf1.shape}", flush=True)
...
print(f"comp: {X.shape}, orig: {X_orig.shape}, fastf1: {X_ff.shape}, test: {X_test.shape}", flush=True)
