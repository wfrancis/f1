...
print(f"\n=== v11_xgb OOF AUC: {cv_auc:.6f} ===")
print(f"elapsed: {time.time()-t0:.1f}s")

sub = pd.DataFrame({"id": ids_test, "PitNextLap": test_pred})
sub.to_csv("submissions/v11_xgb.csv", index=False)
np.save("submissions/v11_xgb_oof.npy", oof)
np.save("submissions/v11_xgb_test.npy", test_pred)
print("submission: submissions/v11_xgb.csv")
